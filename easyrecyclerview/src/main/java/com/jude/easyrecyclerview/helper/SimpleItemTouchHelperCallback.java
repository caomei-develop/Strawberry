...
public class SimpleItemTouchHelperCallback extends ItemTouchHelper.Callback {
	...
	public int getMovementFlags(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
		...
		if (recyclerView.getLayoutManager() instanceof GridLayoutManager) {
			final int dragFlags;
			if ("extraView".equals(viewHolder.itemView.getTag())) {
				dragFlags = 0;
			} else {
	...
	public void onChildDraw(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, float dX,
		...
		if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
			...
			setAlpha(viewHolder.itemView, alpha);
			setTranslationX(viewHolder.itemView, dX);
		} else {
	...
	public void clearView(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
		...
		setAlpha(viewHolder.itemView, ALPHA_FULL);

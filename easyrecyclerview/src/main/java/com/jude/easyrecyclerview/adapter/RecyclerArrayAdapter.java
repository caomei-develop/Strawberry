...
abstract public class RecyclerArrayAdapter<T> extends RecyclerView.Adapter<BaseViewHolder> {
	...
	public void set(int position, T object) {
		if (object != null) {
			synchronized (mLock) {
				if (mObjects != null && mObjects.size() > position)
	...
	public void addAllWithClear(Collection<? extends T> collection) {
		if (collection != null && collection.size() != 0) {
			synchronized (mLock) {
				mObjects.clear();
				mObjects.addAll(collection);
	...
	public void stopMore() {
		getEventDelegate().stopLoadMore();
	}

	public void pauseMore() {
		getEventDelegate().pauseLoadMore();
	}

	public void resumeMore() {
		getEventDelegate().resumeLoadMore();
	}
	...
	public void resetEventDelegate() {
		getEventDelegate().reset();
	}
	...
	private void notifyDataRemoved(final int position) {
	...
	public void swap(final int fromPosition, final int toPosition) {
		...
		if (mNotifyOnChange) {
			notifyDataMove(fromPosition, toPosition);
		}
	}

	private void notifyDataMove(final int fromPosition, final int toPosition) {
	...
	public void move(final int fromPosition, final int toPosition) {
		synchronized (mLock) {
			T object = mObjects.get(fromPosition);
			mObjects.remove(object);
	...
	public final BaseViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
		...
		if(mItemViewClickListener!=null){
            viewHolder.itemView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    mItemViewClickListener.onItemViewClick(v,viewHolder.getAdapterPosition() - headers.size());
                }
            });
        }
	...
	public final void onBindViewHolder(BaseViewHolder holder, int position) {
		...
		holder.itemView.setId(position - headers.size());
	...
	public final int getItemViewType(int position) {
		...
		if (footers.size() != 0) {
			/*
			 * eg: 0:header1 1:header2 2 2:object1 3:object2 4:object3 5:object4
			 * 6:footer1 6(position) - 2 - 4 = 0 7:footer2
			 */
			int i = position - headers.size() - mObjects.size();
			if (i >= 0) {
    ...
    public void setOnItemViewClickListener(OnItemViewClickListener mItemViewClickListener) {
